...
[제목 없음](https://www.notion.so/a80285ab78994a73b179267322790775)
...
아직도 dfs, bfs문제를 풀줄 모른다. 열심히 해야할듯.

이 문제는 BFS로 푸는 문제지만, DFS로 한게 아까워서 이 풀이를 적어본다.

(이 풀이를 제출할경우 시간초과가 난다.) 
...
for(int i = 0; i<4;i++){
	int nx = x+dx[i]; // 
	int ny = y+dy[i];//
	//이 방식으로 4개 방향을 다 가볼 수 있도록 한다.
```

1. DFS에서는 방문 했는지 확인하기 위해 visit리스트를 쓰는 경우가 많다. 

 빈 boolean 리스트를 선언해서, 방문한곳은 false로 바꿔준다.
...
class Solution {
    ...
    int[] dx = {-1, 0, 0, 1};
    int[] dy = {0, 1, -1, 0};
    public int solution(int[][] maps,boolean visit[][]) {
        min = Integer.MAX_VALUE;
        dfs(maps, 0, 0, 0,visit);
...
클래스 안에 기본 변수를 선언, count의 최솟값을 넣어줘야 하므로 min을 최고 큰값으로 지정해놓는다. 

도착하지 못한다면 -1을 리턴하는걸 보여줌.

### DFS함수

```java
public void dfs(int[][] maps, int x, int y, int count,boolean visit[][]) {

        count++;
        if (x == maps[x].length - 1 && y == maps.length - 1) {
            min = Math.min(count, min);
            return;
        }
```

dfs함수에 진입했다는건, 한칸 갔다는것 → count++

x,y,가 제일 오른쪽 제일 아래쪽에 도착했다면, 카운트값을 가지고 있을텐데, count의 제일 작은 값을 min에 넣어준다.

```java
for (int i = 0; i < 4; i++) {
            int nx = x+dx[i];
            int ny = y+dy[i];
            if (nx < 0 || nx >= maps[i].length || ny < 0 || ny >= maps[i].length) {
                continue;
            }
```

4방향으로 다 이동해보며, 범위를 벗어난다면 for문에서 탈출시킨다. 

```java
if(!visit[nx][ny] && maps[nx][ny] ==1){
                visit[nx][ny] = true;
                dfs(maps,nx,ny,count,visit);
                visit[nx][ny]=false;
            }
        }
    }
...
방문하지 않고, 공간에 1이 들어있다면, 

방문처리하고, 방문한곳에서 다시 똑같은 작업을 진행한다.

한번 순회가 다 끝나면, 다시 visit을 방문 안했던걸로 바꿔준다. (경우가 하나밖에 없는건 아니니까!)